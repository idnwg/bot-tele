...
# ========== MEGA-CMD SNAP COMPATIBILITY ==========

def setup_environment():
    """Setup environment untuk snap installation"""
    try:
        # Tambahkan snap bin directories ke PATH
        snap_paths = [
            '/snap/bin',
...
def check_mega_cmd():
    """Check jika mega-cmd terinstall - SNAP COMPATIBLE"""
    try:
        setup_environment()  # Setup environment first
        ...
        # Cek snap installation secara spesifik
        try:
            ...
            if result.returncode == 0 and 'mega-cmd' in result.stdout:
                logger.info("Mega-cmd found in snap list, but commands not in PATH")
                return True
        except:
            pass
...
def get_mega_command():
    """Dapatkan command mega yang work - SNAP COMPATIBLE"""
    try:
...
def run_mega_command(command_args, timeout=60):
    ...
    try:
        ...
        result = subprocess.run(
            ...
            text=True, 
            timeout=timeout,
            env=os.environ  # Gunakan environment yang sudah disetup
        )
...
def check_mega_login():
    """Check status login Mega.nz - SNAP COMPATIBLE"""
    try:
...
def login_to_mega(email, password):
    """Login ke Mega.nz - SNAP COMPATIBLE"""
    try:
        ...
        try:
            ...
            # Gunakan subprocess langsung untuk interactive login
            process = subprocess.Popen(
                [get_mega_command().split()[0] if ' ' in get_mega_command() else get_mega_command(), 'login'],
        ...
        return False, "All login methods failed. Please check your credentials and try manual login with: mega-login email password"
    ...
    try:
        ...
        if not check_mega_cmd():
            await update.message.reply_text(
                ...
                "2. Coba restart bot setelah install\n"
                "3. Atau coba login manual di terminal: `mega-login email password`"
            )
            return
        ...
        if not ensure_mega_session():
            await update.message.reply_text(
                ...
                "Atau coba login manual di terminal:\n"
...
async def download_mega_folder(folder_name, job_id, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = run_mega_command(['get', f'/{folder_name}', target_path], timeout=7200)
    ...
    if save_mega_credentials(email, password):
        ...
        if login_success:
            ...
            await update.message.reply_text(
                ...
                f"**Tips:** Coba login manual di terminal dengan:\n"
...
def main():
    ...
    logger.info("Checking Mega.nz setup with snap compatibility...")
    if check_mega_cmd():
    ...
    # Jalankan bot
    logger.info("🤖 Bot started successfully with Full Snap Compatibility")
