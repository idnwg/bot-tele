...
def check_mega_login():
    """Check status login Mega.nz menggunakan mega-cmd"""
    try:
        ...
        logger.debug(f"mega-cmd whoami failed with output: {result.stdout}, stderr: {result.stderr}")
        return False, "Not logged in"
    except Exception as e:
        return False, f"Error: {str(e)}"

def login_to_mega(email, password):
    """Login ke Mega.nz menggunakan mega-cmd"""
    try:
        logger.info(f"Attempting login to Mega.nz for: {email}")
        # Method: Login dengan mega-cmd
        try:
            ...
            if result.returncode == 0:
                ...
                if is_logged_in:
                    ...
                    logger.warning(f"Session tidak valid setelah login: {msg}")
            else:
...
async def list_mega_folders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = subprocess.run(['mega-cmd', 'ls', '/'], capture_output=True, text=True, timeout=60)
        if result.returncode != 0:
...
async def download_mega_folder(folder_name, job_id, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = subprocess.run(
            ['mega-cmd', 'get', f'/{folder_name}', '-c', target_path],
...
async def upload_to_terabox(folder_path, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if success_count > 0 and (user_settings.get('auto_cleanup', True) or AUTO_CLEANUP):
            await send_progress_message(chat_id, "->{$page_size} Membersihkan folder lokal...", context)
